...
export type TMenuProductInventory = {
	isActive: boolean;
	price: number;
...
export function useMenuProductInventory() {
	...
	useEffect(() => {
		...
		async function loadInventory() {
			...
			if (!isActive) return;
			if (data) setInventory(new Map((data as TMenuProductRow[]).map(product => [product.product_key, toInventory(product)])));
			setIsLoading(false);
		}

		void loadInventory();
		const channelName = `public-menu-inventory-${crypto.randomUUID()}`;
...
function toInventory(product: TMenuProductRow): TMenuProductInventory {
	return { isActive: product.is_active, price: product.price, stockQuantity: product.stock_quantity };
}
