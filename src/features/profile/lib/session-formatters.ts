...
export type TWebDeviceRegistration = {
	deviceName: string;
	platform: string;
};
...
export function getSessionDeviceInfo(
	...
	const details = [browser, operatingSystem].filter(Boolean).join(' · ');
...
export function getWebDeviceRegistration(
	userAgent: string,
): TWebDeviceRegistration {
	return {
		deviceName: getBrowser(userAgent) || 'Web browser',
